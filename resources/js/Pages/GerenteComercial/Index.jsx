...
export default function Index({ contadores, ultimasCotizaciones }) {
    return (
        <GerenteComercialLayout header="Dashboard">
            <Head title="Dashboard Gerente Comercial" />

            <div className="mb-4 flex justify-end gap-3">
                <Link
                    href={route('gerente-comercial.clientes.create')}
                    className="rounded-md border border-[#042753] px-4 py-2 text-sm font-semibold text-[#042753] hover:bg-gray-50"
                >
                    + Nuevo Cliente
                </Link>
                <Link
                    href={route('gerente-comercial.personal.create')}
                    className="rounded-md border border-[#042753] px-4 py-2 text-sm font-semibold text-[#042753] hover:bg-gray-50"
                >
                    + Nuevo Comercial
                </Link>
                <Link
                    href={route('gerente-comercial.cotizaciones.create')}
            ...
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
                <StatCard label="Clientes" value={contadores.clientesActivos} />
                <StatCard label="Comerciales Activos" value={contadores.comercialesActivos} />
                <StatCard label="Embarques Activos" value={contadores.embarquesActivos} />
                <StatCard label="Cotizaciones Activas" value={contadores.cotizacionesActivas} accent />
                <StatCard label="Aceptadas Este Mes" value={contadores.aceptadasEsteMes} />
                <StatCard label="Cotizaciones por Vencer" value={contadores.porVencer} />
            </div>

            <div className="mt-6 flex items-center justify-between">
                <h2 className="text-sm font-semibold text-[#042753]">Últimas Cotizaciones</h2>
                <Link
                    href={route('gerente-comercial.cotizaciones.index')}
                        ...
                        {ultimasCotizaciones.map((cotizacion) => (
                            <tr
                                ...
                                onClick={() =>
                                    router.visit(
                                        route(
                                            'gerente-comercial.cotizaciones.show',
                            ...
                            >
                                ...
                                <td className="px-4 py-3 text-[#042753]">{cotizacion.comercial}</td>
                                <td className="px-4 py-3">
                                    <ModoTransporteBadge modo={cotizacion.modo_transporte} />
                                </td>
                                <td className="px-4 py-3">{cotizacion.fecha_validez}</td>
                                <td className="px-4 py-3">
                                    <span
